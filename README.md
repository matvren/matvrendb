...
MatvrenDB lets you search for a game by name or Steam App ID and download its associated ZIP file if one is available.
It pulls game metadata directly from the Steam store API and checks file availability against the MatvrenDatabase GitHub repository.
...
- ZIP count in the top left, refreshed every minute using the GitHub Git Trees API
